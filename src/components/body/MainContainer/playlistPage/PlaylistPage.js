...
const PlaylistPage = () => {
  ...
  const getPlaylistItemsData = async () => {
    const data = await fetch(
      `https://youtube.googleapis.com/youtube/v3/playlistItems?part=snippet%2CcontentDetails%2Cstatus&maxResults=50&playlistId=${playlistId}&key=${GOOGLE_API_KEY}`
    );
  ...
  const getPlaylistData = async () => {
    const data = await fetch(
      `https://youtube.googleapis.com/youtube/v3/playlists?part=snippet%2CcontentDetails%2Cstatus&id=${playlistId}&key=${GOOGLE_API_KEY}`
    );
    const json = await data.json();
