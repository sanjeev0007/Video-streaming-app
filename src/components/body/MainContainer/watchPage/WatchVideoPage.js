...
const WatchVideoPage = () => {
  ...
  const getVideoData = async () => {
    const data = await fetch(
      `https://youtube.googleapis.com/youtube/v3/videos?part=snippet%2CcontentDetails%2Cstatistics%2Cstatus&id=${videoId}&key=${GOOGLE_API_KEY}`
    );
    ...
    if(json && json.items && json.items.length>0)
    {
      ...
      getComments();
      // console.log(json?.items?.items[0]?.snippet?.title);
