...
import { GOOGLE_API_KEY, YOUTUBE_API_POPULAR_VIDEO_URL } from "../../../../utils/constants/constants";
...
const VideosContainer = () => {
  ...
  const getData = async () => {
    const data = await fetch(
      `https://youtube.googleapis.com/youtube/v3/videos?part=snippet%2CcontentDetails%2Cstatistics&chart=mostPopular&regionCode=${locationCode}&maxResults=30&videoCategoryId=${categoryCode}&key=${GOOGLE_API_KEY}`
    );
  ...
  const getMoreData = async () => {
    ...
    const data = await fetch(
      `https://youtube.googleapis.com/youtube/v3/videos?part=snippet%2CcontentDetails%2Cstatistics&chart=mostPopular&regionCode=${locationCode}&maxResults=15&videoCategoryId=${categoryCode}&key=${GOOGLE_API_KEY}&pageToken=${nextPageToken}`
    );
    const json = await data.json();
